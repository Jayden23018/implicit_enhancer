...
export class Prompter {
    ...
    _createEnhancer(profile) {
        if (profile && profile.name) {
            if (profile.name == "mc_ai-enhancer") {
                this.enhancer_name = profile.name;
                console.log(`[Prompter] Using enhancer: ${profile.name}`);
                return new Enhancer(profile);
            } else if (profile.name == "implicit_enhancer") {
                this.enhancer_name = profile.name;
                console.log(`[Prompter] Using enhancer: ${profile.name}`);
                return new ImplicitEnhancer({ ...profile, agent: this.agent });
            } else {
                console.log("Invalid enhancer name. Use default enhancer.");
                this.enhancer_name = "mc_ai-enhancer";
                return new Enhancer(profile);
            }
        }
    ...
    async promptConvo(messages) {
        ...
        for (let i = 0; i < 3; i++) { // try 3 times to avoid hallucinations
            ...
            console.log(`[Prompter] convo prompt with enhancer=${this.enhancer_name} | messages=${messages.length} | preview="${prompt.slice(0, 200)}..."`);
    ...
    async promptCoding(messages) {
        ...
        console.log(`[Prompter] coding prompt with enhancer=${this.enhancer_name} | messages=${messages.length} | preview="${prompt.slice(0, 200)}..."`);
